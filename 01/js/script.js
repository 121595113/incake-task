...
//
	...
	$('input[name=ck]').change(function() {
		...
		$.ajax({
			...
			data: {
				"type": "_qiefen",
				"_CommodityID": $priceNum,
				"_qiefen": $qiefen
			},
			success: function(data) {
				if (data.msg == "设置成功") {} else {
					alert(data.msg);
				}
			},
			error: function() {
	...
	$(".doublecity .buy_info .go_buy .fn_left").click(function() {
		...
		if (CheckCake()) {
			$.ajax({
				...
				data: {
					"type": "_car",
					"_CommodityID": $("#key").val(),
					"_GuiGe": $("#hid_guigue").val(),
					"_Num": $(this).parents(".doublecity").find(".a_num").html(),
					"cake1": $("#hidcake1").val(),
					"cake2": $("#hidcake2").val()
				},
				success: function(data) {
	...
	$(".goodsdetail_page .buy_info .go_buy .fn_left").click(function() {
			...
			$.ajax({
				...
				data: {
					...
					"_CommodityID": $("#key").val(),
					"_GuiGe": $("#hid_guigue").val(),
					"_Num": $(this).parents(".goodsdetail_page").find(".a_num").html()
				},
				success: function(data) {
					if (data.msg == "加入购物车成功") {
						var cainfo = "[{id:\"" + $("#key").val() + "-" + $("#hid_guigue").val() + "\",n:\"" + $Num + "\",p:\"" + $price + "\"}]";
	...
	$(".go_btn").click(function() {
		...
		if (list.length == 0) {
			...
			//  settle(info);
			if ($("#hid").val() == "0") {
				location.href = "/FirOrder.html";
			} else {
				location.href = "/manage/FirOrder.html";
			}
		}
	})
 ...
 */
$(function(){
	$(".box_ a").click(function() {
		$(".dialog").show();
		$(".details dl").addClass("hover_");
//		添加标识
		$(".box_ a").removeClass("this_box");
		$(this).addClass("this_box");
	})
	$(".details dd a").click(function(){
		$(this).addClass("action")
		var $src=$(".action img").attr("src")
		$(".this_box img").attr("src",$src)
	})
		//重置
	$(".ping_txt .re").click(function() {
		$(".box_").hide();
		$(".c4").show();
		$(".box_ img").attr("src","pic/4wenhao.png");
		$(".a09 img").attr("src","pic/8zgihuangguan.png")
		$(".box_ a").removeClass("this_box");
		$(".details dl").removeClass("hover_");
	});
})
 ...
 */
$(function() {
		$(".order_time .ui_input_text").click(function() {
			$(".order_time .dialog").show();
			var j = $(".xingqi ul.action li").length;
			var liw = $(".xingqi ul.action li").width();
			$(".xingqi ul.action").width(j * (liw + 6))
			$(".order_time .dialog h3 span").click(function() {
				$(".order_time .dialog h3 span").removeClass("moncolor");
				var s = $(".order_time .dialog h3 span").index($(this));
				$(".order_time .dialog h3 span").eq(s).addClass("moncolor");
				$(".xingqi ul").hide().removeClass("action");
				$(".xingqi ul").eq(s).show().addClass("action");
				$(".xingqi ul").css({
					marginLeft: 0
				})
				var j = $(".xingqi ul.action li").length;
				...
				$(".dialog .close").click(function() {
					$(".order_time .dialog").hide()
				})
				gaoliang(); //调用月日星期点击事件函数
			});
			//右滑动
			var n = 0
			$(".xingqi .right").click(function() {
					n++;
					var liw = $(".xingqi ul.action li").width();
					var j = $(".xingqi ul.action li").length;
					if (n > (j - 7)) {
			...
			function gaoliang() {
				$(".xingqi ul.action li").bind("click", function() {
					var s = $(".xingqi ul.action li").index($(this));
					$(".xingqi ul.action li").removeClass("moncolor");
					$(".xingqi ul.action li").eq(s).addClass("moncolor")
				})
			}
			gaoliang(); //调用月日星期点击事件函数
		});
 ...
 */
$(function() {
	$(".birthdaycar ul li:last-child").mousemove(function() {
		$(".birthdaycar #zd01").show();
		$(".birthdaycar #zd02").css("display", "block");
	});
	$(".birthdaycar ul li:last-child").mouseout(function() {
		$(".birthdaycar #zd01").hide();
		$(".birthdaycar #zd02").hide()
	});
	$(".birthdaycar>input").click(function() {
		$(".birthdaycar ul").toggle()
	});
	$(".bir_txt").click(function() {
...
*/
$(function(){
	$(".pic ul div a").live("click",function(){
